    ...
    const fetchThreads = useCallback(async () => {
      ...
      try {
        ...
        type ThreadSearchItem = {
          ...
          created_at: string;
          updated_at?: string;
          values?: { messages?: unknown[] } | null;
        };
        const threadList: Thread[] = (response as ThreadSearchItem[]).map((thread) => {
            ...
            if (
              ...
              if (Array.isArray(messages) && messages.length > 0) {
                displayContent = extractStringFromMessageContent(
                  messages[0] as unknown as import("@langchain/langgraph-sdk").Message,
                );
              }
            }
          } catch (error) {
