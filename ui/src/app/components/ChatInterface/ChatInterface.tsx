...
export const ChatInterface = React.memo<ChatInterfaceProps>(
  ({
    ...
    const [isThreadHistoryOpen, setIsThreadHistoryOpen] = useState(true);
    ...
    */
      ...
      messages.forEach((message: Message) => {
        ...
        if (message.type === "ai") {
          ...
          const toolCallsInMessage: ToolCallLike[] = [];
          if (
            message.additional_kwargs?.tool_calls &&
            Array.isArray(message.additional_kwargs.tool_calls)
          ) {
            toolCallsInMessage.push(
              ...(message.additional_kwargs.tool_calls as unknown as ToolCallLike[]),
            );
          } else if (message.tool_calls && Array.isArray(message.tool_calls)) {
            toolCallsInMessage.push(
              ...((message.tool_calls as unknown as ToolCallLike[]).filter(
                (toolCall) => (toolCall.name || toolCall.type || "") !== "",
              )),
            );
        ...
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            <Image
              ...
              priority
            />
            <h1 className={styles.title}>Learning Agent</h1>
          </div>
